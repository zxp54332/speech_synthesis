...
class LinguisticSource(FileDataSource):
    ...
    def collect_files(self):
        # this repository based on label sate align
    ...
    def collect_features(self, path):
        ...
        if self.add_frame_features:
            indices = labels.silence_frame_indices().astype(
                np.int)  # Returns silence frame indices
        else:
            indices = labels.silence_phone_indices()  # Returns phone-level frame indices
...
class AcousticSource(FileDataSource):
    ...
    def collect_files(self):
        ...
        label_paths = sorted(
            glob(join(self.data_root, label_dir_name, "*.lab")))
        if self.max_files is not None and self.max_files > 0:
    ...
    def collect_features(self, wav_path, label_path):
        ...
        mgc = pysptk.sp2mc(
            spectrogram, order=hp_acoustic.order, alpha=self.alpha)
        f0 = f0[:, None]
        lf0 = f0.copy()
        ...
        if hp_acoustic.mod_spec_smoothing:
            ...
            modfs = fs / hop_length
            T, D = mgc.shape
            DFT_length = 4096 if not (
                (T-4096) > 0) else 4096+np.ceil((T-4096)/2)*2
            mgc = P.modspec_smoothing(
                mgc, modfs, n=int(DFT_length), cutoff=hp_acoustic.mod_spec_smoothing_cutoff)
...
if __name__ == "__main__":
    ...
    if args['--lang'] == 'en':
        ...
        hp_duration.question_path = hp_duration.en_question
    else:
        # cn
        hp_acoustic.question_path = hp_acoustic.cn_question
        hp_duration.question_path = hp_duration.cn_question
    ...
    # hp_acoustic.question_path=join(DATA_ROOT,'questions_dnn.hed')
    ...
    if not skip_duration_feature_extraction:
        ...
        for idx in tqdm(range(len(X_duration))):
            ...
            # need padding zero because chinese language has parameter(subphone_features) problem
            diff = len(y)-len(x)
            if diff:
                x = np.append(x, np.zeros((diff, x.shape[-1])), 0)
            np.save(xpath, x)
            np.save(ypath, y)
    else:
