...
def gen_parameters(y_predicted, Y_mean, Y_std, mge_training=True):
    ...
    if mge_training:
        ...
        vuv = P.inv_scale(
            vuv, Y_mean[ty][vuv_start_idx], Y_std[ty][vuv_start_idx])
    else:
...
def gen_waveform(y_predicted, Y_mean, Y_std, post_filter=False, coef=1.4,
    ...
    mgc, lf0, vuv, bap = gen_parameters(
        y_predicted, Y_mean, Y_std, mge_training)
    ...
    aperiodicity = pyworld.decode_aperiodicity(
        bap.astype(np.float64), fs, fftlen)
    f0 = lf0.copy()
    f0[vuv < 0.5] = 0
...
def gen_duration(label_path, duration_model, X_min, X_max, Y_mean, Y_std):
    ...
    duration_predicted = duration_model(x).data.cpu().numpy()
...
def tts_from_label(models, label_path, X_min, X_max, Y_mean, Y_std,
    ...
    x = _generator_input(hp_duration, x)
    x = x.cuda() if use_cuda else x
    acoustic_predicted = acoustic_model(x).data.cpu().numpy()
...
def get_lab_files(data_dir, label_dir, test=False):
    if test:
        files = NPYDataSource(join(data_dir, "X_acoustic"),
                              test=True).collect_files()
    else:
        files = NPYDataSource(join(data_dir, "X_acoustic"),
                              train=False).collect_files()
...
if __name__ == "__main__":
    ...
    if args['--lang'] == 'en':
        # en
        hp_acoustic.question_path = hp_acoustic.en_question
    else:
        # cn
        hp_acoustic.question_path = hp_acoustic.cn_question
        hp_duration.stream_sizes = [1]
        hp_acoustic.subphone_features = None
    ...
    for typ in ["acoustic", "duration"]:
        ...
        models[typ] = getattr(md, hp.generator)(**hp.generator_params)
    ...
    # 1. Evaluation set
    # 2. Test set
    synthesis_dir = join(outputs_dir, 'synthesis')
    if not exists(synthesis_dir):
        os.makedirs(synthesis_dir)
    synthesis_lab_files = glob(join(labels_dir, '*.lab'))
    for label_path in synthesis_lab_files:
        name = basename(data_dir)+'_'+splitext(basename(label_path))[0]
        print(name)
        dst_path = join(synthesis_dir, name+'.wav')
