...
from multistream import get_static_stream_sizes, select_streams
from multistream import recompute_delta_features
...
class NPYDataSource(FileDataSource):
    ...
    def collect_files(self):
        ...
        npy_files = sorted(
            list(map(lambda d: join(self.dirname, d), npy_files)))
...
def get_vc_data_loaders(X, Y, data_mean, data_var):
    ...
    X_train_cache_dataset = MemoryCacheDataset(
        X_train, cache_size=hp.cache_size)
    Y_train_cache_dataset = MemoryCacheDataset(
        Y_train, cache_size=hp.cache_size)
...
def update_discriminator(model_d, optimizer_d, x, y_static, y_hat_static, lengths,
    ...
    # Real
    D_real = nn.parallel.data_parallel(model_d,y_static_adv)
    real_correct_count = ((D_real > 0.5).float() * mask).sum().item()

    # Fake
    D_fake = nn.parallel.data_parallel(model_d,y_hat_static_adv)
...
def update_generator(model_g, model_d, optimizer_g,
    ...
    if adv_w > 0:
        ...
        loss_adv = -(torch.log(nn.parallel.data_parallel(model_d,y_hat_static_adv) + eps) * mask).sum() / T
    else:
...
def apply_generator(model_g, x, R, lengths):
    if model_g.include_parameter_generation():
        ...
        y_hat, y_hat_static = nn.parallel.data_parallel(model_g,(x, R))
    else:
        # Case: generic models (can be sequence model)
        assert hp.has_dynamic_features is not None
        y_hat = nn.parallel.data_parallel(model_g,x)
...
def compute_distortions(y_static, y_hat_static, Y_data_mean, Y_data_std, lengths=None):
    if hp.name == "acoustic":
        ...
        static_dim = hp.order
        y_static_invscale = P.inv_scale(
            y_static, Y_data_mean[:static_dim], Y_data_std[:static_dim])
        y_hat_static_invscale = P.inv_scale(
        ...
        for phase in ["train", "test"]:
            ...
            for x, y, lengths in dataset_loaders[phase]:
                ...
                if reference_discriminator is not None:
                    ...
                    # actually generated.
                    regard_fake_as_natural += ((target >
                                                0.5).float() * mask).sum().item()
                ...
                if update_g:
                    adv_w = w_d * \
                        float(np.clip(E_loss_mge / E_loss_adv, 0, 1e+3))
            ...
            if update_d and update_g and phase == "train":
                ...
                log_value("MGE/ADV loss weight",  E_loss_mge /
                          E_loss_adv, global_epoch)
...
if __name__ == "__main__":
    ...
    # check checkpoint folder & create the folder
    if not exists(checkpoint_dir):
        os.makedirs(checkpoint_dir)

    # get datasets
    # X=>Linguistic data
    # Y=>Acoustic & Duration data
    X = {"train": {}, "test": {}}
    Y = {"train": {}, "test": {}}
    ...
    # Models
    model_g = getattr(md, hp.generator)(**hp.generator_params)
    model_d = getattr(md, hp.discriminator)(
        **hp.discriminator_params)
    ...
    if checkpoint_path_r is not None:
        reference_discriminator = getattr(
            md, hp.discriminator)(**hp.discriminator_params)
        try:
